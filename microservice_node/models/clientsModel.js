...
module.exports ={
	...
	update: function(user,table,callback){
		console.log('ok');
		var sql ="";
		if(table=='leads') sql="update "+table+" set name='"+user.name+"' , email='"+user.email+"' , phone='"+user.phone+"' , status='"+user.status+"', gender='"+user.gender+"', address='"+user.address+"' where id = '"+user.id+"' ;";
	...
	search: function(email,callback){
		console.log(email);
		var sql= "select * from leads where email= '"+email+"'";
		db.getResults(sql,function(results){
			if(results.length>0)
			{
				callback(true);
			...
			{
				var sql="select * from customer where customerEmail= '"+email+"'";
				db.getResults(sql,function(results){
					if(results.length>0)
					{
						callback(true);
